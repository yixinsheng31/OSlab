...
run_qemu() {
    ...
    if [ -n "$brkfun" ]; then
        ...
        #这里修改成  
        brkaddr_phys=`echo $brkaddr | sed "s/^ffffffffc0/00/g" | sed "s/^c0/00/g"`
...
build_run() {
    ...
    # create icore.img symlink if it doesn't exist
    if [ ! -f bin/icore.img ] && [ -f bin/ucore.img ]; then
        ln -sf ucore.img bin/icore.img
    fi
