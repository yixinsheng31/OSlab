...
void idt_init(void) {
    ...
    write_csr(sscratch, 0);//将 sscratch 寄存器设置为 0，表示当前在内核模式下运行
...
//SSTATUS_SPP 是 sstatus 寄存器中的一个位，表示当前的特权级。
//如果该位为 1，则表示中断发生在内核模式。
...
//根据 tf->cause 的值判断是中断还是异常，并分发到对应的处理函数。
